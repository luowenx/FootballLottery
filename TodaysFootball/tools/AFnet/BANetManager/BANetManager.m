...
//    LQLog(@"\n********************************************************");
    ...
    if (isNeedCache && responseCacheData != nil)
    {
        ...
        LQLog(@"取用缓存数据结果： *** %@", responseCacheData);
...
//        return nil;
    ...
    if (type == BAHttpRequestTypeGet)
    {
        sessionTask = [BANetManagerShare.sessionManager GET:URLString parameters:parameters  progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            // 对数据进行异步缓存
            if ([responseObject isKindOfClass:[NSDictionary class]] && responseObject[@"ret"]) {
                
                if ([responseObject[@"ret"] integerValue] == kLQNetResponseSuccess && isNeedCache) {
            ...
            if(((NSHTTPURLResponse *)task.response).statusCode == kLQNetErrorCodeNeedGetBoot){[[LQAppConfiger shareInstance] forceBoot];}
    ...
    else if (type == BAHttpRequestTypePost)
    {
        sessionTask = [BANetManagerShare.sessionManager POST:URLString parameters:parameters progress:^(NSProgress * _Nonnull uploadProgress) {
            LQLog(@"上传进度--%lld, 总进度---%lld",uploadProgress.completedUnitCount,uploadProgress.totalUnitCount);
            ...
            LQLog(@"post 请求数据结果： *** %@", responseObject);
            if (successBlock)
            {
                successBlock(responseObject);
            }
            ...
            LQLog(@"错误信息：%@",error);
            NSLog(@"----%ld", ((NSHTTPURLResponse *)task.response).statusCode);
            if(((NSHTTPURLResponse *)task.response).statusCode == kLQNetErrorCodeNeedGetBoot){[[LQAppConfiger shareInstance] forceBoot];}
        ...
        [imageArray enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if ([obj isKindOfClass:[PHAsset class]])
            {
                ...
                [imageManager requestImageForAsset:asset targetSize:CGSizeMake(asset.pixelWidth , asset.pixelHeight) contentMode:PHImageContentModeAspectFit options:nil resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
                    ...
                    LQLog(@" width:%f height:%f",result.size.width,result.size.height);
...
//    LQLog(@"******************************************************");
    ...
    sessionTask = [BANetManagerShare.sessionManager downloadTaskWithRequest:downloadRequest progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        LQLog(@"下载进度：%.2lld%%",100 * downloadProgress.completedUnitCount/downloadProgress.totalUnitCount);
        /*! 回到主线程刷新UI */
        dispatch_async(dispatch_get_main_queue(), ^{
                             ...
                             index:(NSUInteger)index
{
    ...
    {   // 图片数据不为空才传递 fileName
        ...
        LQLog(@"上传图片 %lu 成功", (unsigned long)index);
    }
}
