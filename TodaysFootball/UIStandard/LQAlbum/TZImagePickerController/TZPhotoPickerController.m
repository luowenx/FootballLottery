...
//    if (iOS7Later) top += 20;
    CGFloat top = 0;
    CGFloat collectionViewHeight = tzImagePickerVc.maxImagesCount > 1 ? self.view.tz_height - 50 - top : self.view.tz_height - 64;
        ...
        if (tzImagePickerVc.maxImagesCount == 1) {
            [[TZImageManager manager] getPhotoWithAsset:model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
                if (!isDegraded) {
                    ...
                    clipper.originalImage = photo;
                    clipper.imageSize = CGSizeMake(800, 800);
                    clipper.uploadHandle = ^(UIImage *image, id value) {
                        if (tzImagePickerVc.pickerDelegate && [tzImagePickerVc.pickerDelegate respondsToSelector:@selector(imagePickerController:didUploadImage:res:)]) {
                            [tzImagePickerVc.pickerDelegate imagePickerController:tzImagePickerVc didUploadImage:image res:value];
                        }
                        [self.navigationController dismissViewControllerAnimated:YES completion:nil];
                    };
