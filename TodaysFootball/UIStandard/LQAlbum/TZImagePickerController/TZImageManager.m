...
//        PHFetchResult *topLevelUserCollections = [PHCollectionList fetchTopLevelUserCollectionsWithOptions:nil];
        ...
        for (PHAssetCollection *collection in smartAlbums) {
            ...
            if ([collection.localizedTitle containsString:@"Panoramas"] || [collection.localizedTitle isEqualToString:(@"全景照片")]) continue;
            if ([self isCameraRollAlbum:collection.localizedTitle]) {
...
- (void)savePhotoWithImage:(UIImage *)image completion:(void (^)(NSError *error))completion {
    ...
    if (@available(iOS 9.0, *)) { // 这里有坑... iOS8系统下这个方法保存图片会失败
...
- (TZAlbumModel *)modelWithResult:(id)result name:(NSString *)name{
    ...
    if ([result isKindOfClass:[PHFetchResult class]]) {
//        PHFetchResult *fetchResult = (PHFetchResult *)result;
        model.count = model.models.count;
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    } else if ([result isKindOfClass:[ALAssetsGroup class]]) {
        ALAssetsGroup *group = (ALAssetsGroup *)result;
